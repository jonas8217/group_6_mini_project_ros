...
u32 XInfer_Get_return(XInfer *InstancePtr) {
    ...
    Data = XInfer_ReadReg(InstancePtr->Control_BaseAddress, XINFER_CONTROL_ADDR_AP_RETURN);
    return Data;
}
u32 XInfer_Get_in_r_BaseAddress(XInfer *InstancePtr) {
    ...
    return (InstancePtr->Control_BaseAddress + XINFER_CONTROL_ADDR_IN_R_BASE);
}
...
u32 XInfer_Get_in_r_HighAddress(XInfer *InstancePtr) {
    ...
    return (InstancePtr->Control_BaseAddress + XINFER_CONTROL_ADDR_IN_R_HIGH);
}
...
u32 XInfer_Get_in_r_TotalBytes(XInfer *InstancePtr) {
    ...
    return (XINFER_CONTROL_ADDR_IN_R_HIGH - XINFER_CONTROL_ADDR_IN_R_BASE + 1);
}
...
u32 XInfer_Get_in_r_BitWidth(XInfer *InstancePtr) {
    ...
    return XINFER_CONTROL_WIDTH_IN_R;
}
...
u32 XInfer_Get_in_r_Depth(XInfer *InstancePtr) {
    ...
    return XINFER_CONTROL_DEPTH_IN_R;
}
...
u32 XInfer_Write_in_r_Words(XInfer *InstancePtr, int offset, word_type *data, int length) {
    ...
    if ((offset + length)*4 > (XINFER_CONTROL_ADDR_IN_R_HIGH - XINFER_CONTROL_ADDR_IN_R_BASE + 1))
        return 0;

    for (i = 0; i < length; i++) {
        *(int *)(InstancePtr->Control_BaseAddress + XINFER_CONTROL_ADDR_IN_R_BASE + (offset + i)*4) = *(data + i);
    }
    return length;
}
...
u32 XInfer_Read_in_r_Words(XInfer *InstancePtr, int offset, word_type *data, int length) {
    ...
    for (i = 0; i < length; i++) {
        *(data + i) = *(int *)(InstancePtr->Control_BaseAddress + XINFER_CONTROL_ADDR_IN_R_BASE + (offset + i)*4);
    }
    return length;
}
...
u32 XInfer_Write_in_r_Bytes(XInfer *InstancePtr, int offset, char *data, int length) {
    ...
    if ((offset + length) > (XINFER_CONTROL_ADDR_IN_R_HIGH - XINFER_CONTROL_ADDR_IN_R_BASE + 1))
        return 0;

    for (i = 0; i < length; i++) {
        *(char *)(InstancePtr->Control_BaseAddress + XINFER_CONTROL_ADDR_IN_R_BASE + offset + i) = *(data + i);
    }
    return length;
}
...
u32 XInfer_Read_in_r_Bytes(XInfer *InstancePtr, int offset, char *data, int length) {
    ...
    for (i = 0; i < length; i++) {
        *(data + i) = *(char *)(InstancePtr->Control_BaseAddress + XINFER_CONTROL_ADDR_IN_R_BASE + offset + i);
    }
    return length;
}
